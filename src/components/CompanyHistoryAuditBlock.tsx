import type { CompanyHistoryEntry } from "../utils/constants";
import {
  companySnapshotToDisplayRows,
  computeCompanyHistoryChanges,
} from "../utils/companyHistoryDisplay";

export function CompanyHistoryAuditBlock({
  entry,
  isDarkMode,
}: {
  ...
  if (phase === "update" && before && after) {
    const changes = computeCompanyHistoryChanges(before, after);
    if (changes.length === 0) {
      return (
  ...
  if ((phase === "create" || phase === "import") && after && Object.keys(after).length > 0) {
    const rows = companySnapshotToDisplayRows(after);
    return (
      <div className={`mt-3 rounded-lg border text-xs overflow-x-auto ${box}`}>
        <p
  ...
  if (phase === "delete" && before && Object.keys(before).length > 0) {
    const rows = companySnapshotToDisplayRows(before);
    return (
      <div className={`mt-3 rounded-lg border text-xs overflow-x-auto ${box}`}>
        <p
          className={`px-2 py-1.5 font-semibold border-b ${
